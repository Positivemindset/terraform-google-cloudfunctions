...
resource "null_resource" "clone_repository" {
  provisioner "local-exec" {
    command = "git clone ${var.github_url} ${var.folder_name}"
  }
}
...
resource "null_resource" "install_packages" {
  ...
  # Trigger the provisioner again if the requirements file changes
  triggers = {
    file_content = sha1(file("${var.folder_name}/requirements.txt"))
  }
}
...
/* resource "null_resource" "cleanup_folder_and_zip" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      if [ -f ${var.zip_name}.zip ]; then
        rm -f ${var.zip_name}.zip
      fi'
    EOT
  }
} */
